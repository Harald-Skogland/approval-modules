...
(function () {
    ...
    var html = cols.map(function (c) {
      if (c.id === 'select') {
        return '<div class="tg-hc tg-hc--plain" data-select-all>' +
          ...
          '<span class="ga-checkbox__marker"></span></label></div>';
      }
      if (c.id === 'actions') {
    ...
    switch (col.id) {
      ...
      case 'documentDueDate': {
        var marker = isOverdue(task)
          ? '<span class="tg-overdue-icon" title="Overdue">' + svg('alert', 16) + '</span>' : '';
        return '<div class="tg-cell"><span class="tg-cell__text">' + fmtDate(task.documentDueDate) + '</span>' + marker + '</div>';
      }
  ...
  function renderColumnsMenu() {
    el.colMenu.innerHTML = COLUMNS.filter(function (c) { return !c.fixed; }).map(function (c) {
      return '<button class="ga-menu__item" data-toggle-col="' + c.id + '" role="menuitemcheckbox" aria-checked="' + !!c.on + '">' +
        ...
        '<span class="ga-checkbox__marker"></span></label>' +
  ...
  el.body.addEventListener('click', function (e) {
    ...
    if (dataRow) toast('Task detail is not part of this prototype');
  });
